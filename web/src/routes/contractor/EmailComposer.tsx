...
type EditorTab = 'templates' | 'compose' | 'preview';
...
export default function EmailComposer({ workItems, clients }: EmailComposerProps) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      // For invoice emails, update invoice status after successful send
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Editor card */}
      <div className="relative bg-[var(--bg-card)] rounded-xl border border-[var(--border)]">
        {/* Tabs */}
        <div className="flex border-b border-[var(--border)]">
          {(['templates', 'compose', 'preview'] as EditorTab[]).map((t) => {
        ...
        <div>
            ...
            <div className="p-4 space-y-4">
              ...
              >
                ...
                <div className="text-xs text-[var(--text-secondary)] mt-1">
                  Use the default {isInvoice ? 'invoice' : 'completion'} email template
                </div>
              </button>
          ...
          {/* Preview tab */}
          {tab === 'preview' && (
            <div className="bg-[#F5F5F0] rounded-b-xl">
              <iframe
                srcDoc={currentHtml}
                title="Email Preview"
